...
// متغير المؤقت لزر إعادة إرسال OTP
var otpTimer = null;
...
async function startOtpProcess() {
    ...
    db.ref('users/' + fullPhone).once('value', snapshot => {
        ...
        // إذا كان هناك خطأ في صلاحيات قاعدة البيانات، سيخبرك هنا
...
function sendOtpCode(fullPhone, btn) {
    ...
    auth.signInWithPhoneNumber(fullPhone, window.recaptchaVerifier)
        .then((result) => {
            ...
            btn.disabled = false;
            btn.innerText = "إرسال رمز التحقق";
            startOtpTimer(); // تشغيل العداد هنا
        }).catch((error) => {
            // هنا النظام سيكتشف المشكلة ويخبرك بها بالتحديد!
...
function verifyOtpCode() {
    ...
    // تحويل لون زر التأكيد للرصاصي وجعله غير قابل للضغط
    ...
    confirmationResult.confirm(code).then(() => {
        ...
        // إرجاع اللون الأحمر للزر في حالة الفشل ليحاول مجدداً
        vBtn.style.background = 'var(--red)';
        vBtn.innerText = 'تأكيد الرمز';
        vBtn.disabled = false;
        document.getElementById('otp-code').value = ""; // مسح الكود الخاطئ
    });
}
...
function closeOtpOverlay() { 
    ...
    document.getElementById('otp-code').value = ""; // تفريغ خانة OTP عند الإلغاء
    clearInterval(otpTimer);
    
    // إرجاع زر التأكيد لشكله الأصلي للعملية القادمة
    var vBtn = document.getElementById('verify-otp-btn');
    if(vBtn) {
        vBtn.style.background = 'var(--red)';
        vBtn.innerText = 'تأكيد الرمز';
...
function goTo(id) {
    // تفريغ الخانات عند الخروج من صفحة التسجيل
